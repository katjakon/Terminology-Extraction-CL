...
class Preprocess:
    ...
    def get_freq(self, bigram_list, fileid=None):
        if fileid is not None:
            freq_f = self.bigrams_in_file(fileid)
            return {bigram: freq_f[bigram] for bigram in freq_f if bigram in bigram_list}
        return {bigram: self.bigrams.get(bigram, 0) for bigram in bigram_list}

    def bigrams_to_file(self,
                        bigrams,
                        file_dir="preprocess",
                        name="candidates",
                        ext=".txt"):
    ...
    def bigrams_in_file(self, fileid):
